...
const VerifyPhonesCode: React.FC = () =>
{
    ...
    const verifyCode = async (completeCode: string) => {
        try {
            const response = await axios.post(`${API_URL}/api/phone/verifyCode`, {
                code: completeCode,
                phone: phone,
                ...
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}` // Токен для авторизації
                }
            });
    ...
    const resendCode = async () => {
        ...
        await axios.post(`${API_URL}/api/sendCode`, {phone}, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`
            }
        });
                ...
                <div className="icon">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                              d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8m-18 4l7.89 5.26a2 2 0 002.22 0L21 12"/>
                    </svg>
                </div>
                <h2>Confirm your phone number</h2>
                <p>Enter the code we sent you to phone number <strong>{phone}</strong></p>
...
export default VerifyPhonesCode;